...
def validateRegistration(data):
	if frappe.db.exists("User", data.email):
		frappe.throw("البريد الإلكتروني موجود مسبقا")
		
	if frappe.db.exists("Company", data.company):
		frappe.throw("الشركة مسجلة مسبقا")

	id = frappe.get_value("User", filters={"national_id": data.id_number})
	if id is not None:
		frappe.throw(f"{id} رقم الهوية مستخدم من قبل شخص أخر")

	if frappe.get_value("User", filters={"mobile_no": data.phone_number}) is not None:
		frappe.throw("رقم الجوال مستخدم من قبل شخص أخر")

	if frappe.get_value("Company", filters={"abbr": data.abbr}) is not None:
		frappe.throw("الإسم المختصر للشركة موجود مسبقا")
